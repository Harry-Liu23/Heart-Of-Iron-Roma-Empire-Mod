...
包含 68 个帝国国策、23 个自定义决议、18 个事件、14 位指挥官，以及移植的原版偿债决议。完整结构、逐节点效果和各分支累计奖励见 [当前完整国策与效果](docs/当前完整国策与效果.md)。独立军事线尚未设计。
...
静态检查覆盖括号、引用、前置、互斥、州编号、本地化和关键数值。军工机构失去所在地后，已分配生产线与现有设计的引擎行为仍需游戏内确认。
