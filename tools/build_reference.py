        ...
        for ps in f['parents']:
            for parent in ps:
                if parent not in ids and parent not in external:
                    name=index[parent]['name'] if parent in index else {'GRE_metaxism_focus':'梅塔克萨斯主义','GRE_bring_home_the_exiled_republicans':'流亡共和派回国'}.get(parent,parent)
...
lines+=['','原版小额、大额偿债和债务重组决议另保留在帝国财政类别，使用原版债务变量。没有新增拒付全部债务决议。','',
'## 事件文本','']
for e in m['events']:
